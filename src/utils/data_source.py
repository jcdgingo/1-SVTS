...
class FinancialAnalytics:
    ...
    def __init__(self, ticker='FOREX:EUR', date_from='20210404T0130', sort='EARLIEST', file_path=r'C:\Users\jessi\OneDrive\1 Projects\1-svts\data\raw\EURUSD_M1_201101030000_202310201036.csv'):
        ...
        self.sort = sort
# fetch_market_data method has a raw data in TSV file and the code is created to convert it to csv
    def fetch_market_price_data(self):
        file_path = self.file_path       
        df_tsv = pd.read_csv(file_path, sep='\t')
        df_tsv.drop('<VOL>', axis=1, inplace=True)# <VOL> is dropped since FOREX has no real volume, only ticker volume
        ...
        return df_tsv
# fetch market news sentiment from Alpha Vantage API, code for cleaning from nested json format and extract sentiment
    ...
    def cluster_data(self):
        file_path = self.file_path 
        df_cluster = file_path.fetch_market_price_data()
        ...
        plt.ylabel('WCSS')
        plt.show()
        best_k = 6  # Adjust this based on your observation from the elbow graph
